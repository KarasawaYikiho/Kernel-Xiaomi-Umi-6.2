...
set +e
make -C "$KERNEL_DIR" O="$OUT_DIR" LLVM=1 LLVM_IAS=1 "${DEVICE}_defconfig" > "$ARTIFACTS_DIR/make-defconfig.log" 2>&1
rc1=$?

# Core build for packaging/readiness (fatal on failure)
make -C "$KERNEL_DIR" O="$OUT_DIR" LLVM=1 LLVM_IAS=1 -j"$(nproc)" Image.gz modules > "$ARTIFACTS_DIR/make-build.log" 2>&1
rc2=$?
...
if [ -s "$ARTIFACTS_DIR/target_dtb_manifest.txt" ]; then
  while IFS= read -r dtb; do
    ...
    make -C "$KERNEL_DIR" O="$OUT_DIR" LLVM=1 LLVM_IAS=1 "qcom/$dtb" >> "$ARTIFACTS_DIR/make-target-dtbs.log" 2>&1
    r=$?
    if [ "$r" -ne 0 ]; then
      rc3=$r
    fi
  ...
  make -C "$KERNEL_DIR" O="$OUT_DIR" LLVM=1 LLVM_IAS=1 -j"$(nproc)" dtbs > "$ARTIFACTS_DIR/make-target-dtbs.log" 2>&1
  rc3=$?
fi
set -e
