...
- 新增 `tools/porting/build_artifact_checksums.py`，输出 `artifacts/artifact-sha256.txt`，用于产物一致性校验与追溯。
- `phase2-port-umi.yml` 增加 `concurrency`（同分支新运行会取消旧运行）与 `timeout-minutes: 90`，降低卡死与重复运行成本。
