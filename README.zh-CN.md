...
- 说明：
  - `UtsavBalar1231` 与 `liyafe1997`（Strawing）按“作者 ID（账号级）”处理，先做账号级发现，再选择仓库做驱动参考。
  - 新增参考仅用于驱动差异分析与定向移植，不做整树盲拷。
  - 若某作者 ID 下未发现公开 kernel/driver 仓库，会在 inventory 中记录为空结果。

## 官方 ROM 基线（仅分析）

- 已分析基线包：`D:\GIT\MIUI_UMI_OS1.0.5.0.TJBCNXM_d01651ed86_13.0.zip`
- 分析产物：`Porting/OfficialRom-Umi-Os1.0.5.0-Analysis.md`
- 范围：仅保留 metadata/hash/partition-op 证据，不向仓库导入专有 blob

## 工作流

> CI 兼容提示：工作流已设置 `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true`，用于规避 GitHub Runner 上 Node20 弃用告警。
